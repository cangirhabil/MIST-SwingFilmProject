...
public class ReadCSV {
	...
	public ArrayList<Film>  allFilms = new ArrayList<Film>();
	...
	public void fetchFilms(ArrayList<Film> films) {
		...
		try {
			File filmFile = new File("imdb_top_1000.csv");
			...
			while (sc.hasNext()) {
				...
				String posterLink = line[0].toString();
				posterLink = posterLink.substring(1, posterLink.length() - 1);
				
				String title = line[1].toString().trim();		
				...
				String description = line[7].toString().trim();
				...
				if(line[9].toString().toLowerCase().trim() != "") {
					...
					if(	d == null) {
						Film tempFilm = new Film(posterLink,title,releaseDate,certificate,duration,genre,rate,metascore,description,null,null,voteCount,grossRevenue);
				...
				Film film = new Film(posterLink,title,releaseDate,certificate,duration,genre,rate,metascore,description,director,stars,voteCount,grossRevenue);			
